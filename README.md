...
- 2467 (용액)
  - 투포인터 or 이분탐색
  - 투포인터로 해결하였다 최소와 최대지점을 두 포인터로 잡고 둘이 더해서 음수면 왼쪽을 양수면 오른쪽을 움직인다.
  - 이분탐색으로도 가능하다고 한다.
    - 좌측에서 하나씩 움직이면서 가장 비슷한 원소를 우측에서 이분탐색
- 2473 (세 용액)
  - 투포인터
  - 바로 위의 문제에서 용액하나 추가
  - i=0~N-3까지 루프를 돌리면서 나머지로 투포인터로 최소값 찾음
- 2568(전깃줄 - 2)
  - 가장 긴 증가하는 부분 수열 (LIS, Longest Increasing Subsequence)
  - 교차가 없도록 전깃줄을 연결하는 방법은 왼쪽을 정렬해서 봤을때 오른쪽의 연결이 정렬된 상태라는것
    - ex) 왼쪽에 연결이 123456일때 오른쪽이 213456이면 교차 발생 -> x13456이면 교차없음
  - 따라서 LIS알고리즘 사용
    - LIS배열을 만든다
    - 주어진 수 x와 LIS\[-1\] 비교 주어진 수가 더 크다면 LIS에 append 아니라면 LIS요소중 x보다 큰수중 가장 작은수를 찾아 대입
      - ex) 1357일때 2가 주어진다면 1257로 바꾼다.
    - 이렇게 하면 LIS의 길이를 잴 수 있다.
    - 자를 위치를 확인하기 위해서는 추가 배열 필요 (record)
    - 각 요소를 체크하면서 삽입하는 위치를 기록한다
    - 기록후 우측에서 감소하는 순으로 추출하면 종료
      - ex) 0010213415 라면 0(01)0(2)1(34)1(5)으로 괄호가 남겨둬야 할 배열이 됨 따라서 남은것을 출력하고 종료
- 2623(음악프로그램)
  - 위상정렬
  - 위상정렬을 통해 순서를 만드는 문제.
  - 최종 결과의 길이가 입력 n과 다르면 안에 사이클이 발생하는것 -> return 0
  - 위상정렬의 경우 엔트리레벨과 특정 작업과 뒷작업간의 관계만 잘 저장하면 쉽게 돌아간다.
- 7453(합이 0인 네 정수)
  - 딕셔너리?
  - a,b,c,b에 size가 최대 4천이므로 단순하게 시도하면 4000^4로 해결불가
  - n^2으로 내릴방법 -> a,b를 조합한 모든 경우의수를 dictionary에 넣고 c,d로 시도해가면서 dict를 조회한다.
  - 이때 중요한게 같은수가 여러번 만들어질 수 있다는것 dict에는 해당수가 발생한 횟수가 저장되어야한다.
  - defaultdict를 사용하면 조회가 아니라 단순히 +로 바꿀수 있어서 더 편하다.
- 1006(습격자 초라기)
  - dp
  - 까다로운 dp문제.. 여러 케이스를 생각해야한다.
  - 각각 dp_a, dp_b, dp_c 3개의 배열을 n+1 사이즈로 선언한다.
    - a_i: i-1행까지 양쪽 다 채워지는데 필요한 최소 소대
    - b_i: i행의 0번열, i-1행의 양쪽 다 채워지는 (ㄱ을 뒤집은 모양)데 필요한 최소 소대
    - c_i: 위의 반대 (ㄴ모양)
  - 각각의 경우를 채워가면서 한다
  - 그리고 n-1행에서 0행으로 넘어오는 부분은 4가지 경우의수
    - 걸치는 소대가 없다
    - 위쪽만 걸친다
    - 아래쪽만 걸친다
    - 양쪽 다 걸친다.
...
- 10830 (행렬 제곱)
  - 분할정복을 이용한 거듭제곱, 비트마스킹
  - 행렬제곱문제. 분할정복을 이용한다 -> d=10 -> 8 + 2 (b1010)
  - 이때 값으로 1이 들어오는것 처리를 신경써야 한다..
  - ex) [[1000, 1000], [1000, 1000]] 의 1승은 0,0,0,0임을 잊지 않도록
  - result행렬 처음곱은 mat * E를 사용하였다
- 12850 (본대 산책)
  - 인접행렬 거듭제곱
    - 그래프 G(V, E)가 있을때 임의의 정점 a에서 b까지 M개의 간선을 거쳐서 가는 경우의 수를 구하기
    - 문제에서 인접행렬이 주어질때 인접행렬을 구하고 이를 N번 제곱한다.
    - D_(i,j,N)이라면 i에서 j로 N개의 간선을 거쳐가는 경우의 수는 D
- 1761 (정점들의 거리)
  - LCA (최소 공통 조상) 문제
  - 트리 구축할때, 각 노드의 dist, depth, ancestor matrix를 만든다.
    - dist는 루트로부터 거리, depth는 각 노드의 깊이(레벨), ancestor matrix는 각 노드로부터 2^(i-1) 번째 조상노드를 의미함.
  - 각 쿼리가 주어졌을때 두 노드의 깊이를 맞춘뒤(낮은쪽을 올린다 이때 matrix에서 가장 깊은높이부터 이용) 최소공통조상을 찾아서 거리를 만들어 출력
- 6549 (히스토그램에서 가장 큰 직사각형)
  - 분할정복, 세그먼트 트리
  - 을히스토그램 최소값 위치를 저장하는 세그먼트 트리를 만든뒤에 이를 활용한다.
  - 각 범위에서 가장 낮은 히스토그램 위치를 찾은뒤 이를 이용해서 가장 낮은 높이*범위로 넓이를 구하고 이 위치를 기준으로 좌우 분할정복
## SWEA
- 2819
  - DFS, Memoization
